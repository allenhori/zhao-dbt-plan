...
## Making the plan actually apply: `zhao_dbt_plan_macros`
...
[`zhao_dbt_plan_macros`](integrations/dbt_package) is a small, separately-licensed (Apache-2.0)
companion dbt package — `wref()`, a drop-in `ref()` replacement, plus two boundary helpers — that
closes that gap, reading the exact same `meta.zhao` block this planner already does. See its own
README for install/usage; it's optional, and `zhao-dbt-plan` itself works identically without it.
